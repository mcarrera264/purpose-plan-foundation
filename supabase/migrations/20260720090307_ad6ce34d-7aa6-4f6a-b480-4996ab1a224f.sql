...
-- Atomic AI suggestion acceptance
CREATE OR REPLACE FUNCTION public.accept_ai_suggestion(
  _suggestion_id UUID,
  _edited_title TEXT DEFAULT NULL,
  _edited_description TEXT DEFAULT NULL,
  _edited_estimate TEXT DEFAULT NULL
)
...
BEGIN
  ...
  -- Row-lock the suggestion to prevent concurrent double-accept
  ...
  v_estimate := COALESCE(_edited_estimate, v_sug.estimate);
  ...
  -- Duplicate detection against non-archived siblings (case-insensitive, whitespace-normalized)
...
REVOKE ALL ON FUNCTION public.accept_ai_suggestion(UUID, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.accept_ai_suggestion(UUID, TEXT, TEXT, TEXT) TO authenticated;

-- Account deletion: wipes user data but preserves auth identity (client will then sign out).
...
BEGIN
  ...
  DELETE FROM public.ai_suggestions
    WHERE batch_id IN (SELECT id FROM public.ai_suggestion_batches WHERE user_id = v_uid);
  DELETE FROM public.ai_suggestion_batches WHERE user_id = v_uid;
  DELETE FROM public.tasks WHERE user_id = v_uid;
  DELETE FROM public.projects WHERE user_id = v_uid;
  DELETE FROM public.areas WHERE user_id = v_uid;
